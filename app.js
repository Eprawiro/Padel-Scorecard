...
const PHOTO = {
  ...
  'sandy':'generic-padel-avatar.svg'
};
...
function home(){
  ...
  return `<section class="hero"><div class="eyebrow">FLPR Premium Final Production Candidate 2</div><h1>Every Point Matters.</h1><p>Official tournament results, ranking, handicap, player development, statistics, and hall of fame in one stable platform.</p><div class="actions"><a class="button" href="#tournaments">Tournament Center</a><a class="button secondary" href="#scorecard">Player Scorecards</a></div></section>
...
function scorecardDetail(p){
  ...
  const photoNote=p.slug==='sandy'?'<div class="photo-pending">Official Sandy photo has not been supplied in the current file set; placeholder retained to avoid using the wrong person.</div>':'';
...
function board(titleText,players,valueFn,label){return `<section class="leaderboard panel"><div class="panel-head"><h3>${escapeHtml(titleText)}</h3>${badge(label)}</div>${players.slice(0,5).map((p,i)=>`<a href="#scorecard/${encodeURIComponent(p.slug)}" class="leader-row"><b>#${i+1}</b><img src="${avatar(p.name)}" alt=""><span>${escapeHtml(p.name)}</span><strong>${escapeHtml(valueFn(p))}</strong></a>`).join('')}</section>`;}
...
function statistics(){
  ...
  return `${title('Statistics Center','Leaderboards and performance indicators generated from the workbook dataset.')}${stats([['Average Rating',DATA.kpis['Average Rating']],['Rating Spread',DATA.kpis['Rating Spread']],['Balance Index',DATA.kpis['Competitive Balance Index']],['Average Win Rate',pct(Number(DATA.kpis['Average Win Rate'])*100)]])}<div class="leaderboard-grid">${board('Top Movers',improved,p=>movementText(p),'Official snapshot movement')}${board('Momentum Leaders',momentum,p=>momentumScore(p).toFixed(1),'Composite momentum')}${board('Most Consistent',consistent,p=>p.consistency.toFixed(1),'Consistency index')}${board('Highest Win Rate',win,p=>pct(p.winRate),'Minimum 6 matches')}${board('Most Dominant',dominant,p=>p.dominance.toFixed(1),'Dominance index')}${board('Clutch Leaders',clutch,p=>p.clutch.toFixed(1),'Clutch index')}${board('Most Versatile',versatile,p=>p.versatility.toFixed(1),'Partner versatility')}${board('Toughest Schedule',tough,p=>p.sos.toFixed(1),'Schedule strength')}</div><div class="notice"><strong>Top Movers:</strong> the current workbook is the production baseline, so most official movements remain unchanged until a new approved tournament snapshot is imported.</div>`;
}
