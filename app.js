...
function verifiedHistory(p){
  const sortHistory=items=>items.sort((a,b)=>String(a.dateISO||'').localeCompare(String(b.dateISO||''))||tournamentSequence({name:a.label})-tournamentSequence({name:b.label}));
  if(Array.isArray(p.tournamentFinishHistory)&&p.tournamentFinishHistory.length)return sortHistory(p.tournamentFinishHistory.map((x,index)=>({label:tournamentDisplayName({name:x.name,label:x.label,id:x.tournamentId},index),points:Number(x.totalPoints||0),rank:Number(x.finalPosition),date:x.date||x.dateISO||'',dateISO:x.dateISO||''})).filter(x=>Number.isFinite(x.rank)&&x.rank>=1));
  const snapshots=loadImportedTournaments().filter(t=>Array.isArray(t.standings)).map((t,index)=>{const row=t.standings.find(x=>x.playerId===p.id||slugify(x.name)===p.slug);const rank=Number(row?.finalPosition??row?.rank);const points=Number(row?.totalPoints??row?.points??0);return row&&Number.isFinite(rank)&&rank>=1?{label:tournamentDisplayName(t,index),points,rank,date:t.date,dateISO:t.dateISO||''}:null;}).filter(Boolean);
  return sortHistory(snapshots);
}
...
function historicalAnalyticsDashboard(p){
  ...
  return `<section class="player-intelligence-shell">
    ...
    <div class="panel intelligence-section"><div class="section-heading"><div><span>03</span><h3>Career Position</h3>${infoButton('career-position')}</div>${badge('Official')}</div><div class="career-position-grid">${[['Current Official Rank',`#${c.current_rank}`,'rank'],['Career Best Official Rank',`#${c.best_rank_ever}`,'career-best-rank'],['Average Official Rank',Number(c.average_rank||0).toFixed(2),'average-rank-career'],['Official Rank Movement',analyticsMovement(c.rank_change),'rank-movement-career'],['Rating Change',`${Number(c.rating_change||0)>0?'+':''}${Number(c.rating_change||0).toFixed(2)}`,'rating-change-career'],['Career Trend',Number(c.career_trend_score||0).toFixed(1),'career-trend']].map(([k,v,key])=>`<div><small>${parameterLabel(k,key)}</small><strong>${v}</strong></div>`).join('')}</div></div></section>
...
function scenarioPodiumCard(row,index){
  ...
  return `<a class="scenario-podium-card place-${index+1}" href="#scorecard/${encodeURIComponent(row.p.slug)}"><span class="scenario-place">${index+1}</span>${avatarImg(row.p.name,'scenario-avatar',row.p.name)}<div><small>${isRankingEligible(row.p)?'Official Ranking eligible':'Provisional / Emerging'}</small><h3>${escapeHtml(row.p.name)}</h3><span>${officialStatusBadge(row.p)}</span></div><section><b>${row.m.readiness.toFixed(1)}</b><em class="outlook-band ${bandClass}">${band}</em><small class="confidence-dot ${confidenceClass}">${confidence} evidence</small></section></a>`;
}
